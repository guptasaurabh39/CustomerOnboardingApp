							...
							if (mtchAttrId.equals(srcParentAttributeId)) {
								...
								for (int idCnt = 0; idCnt < lstSrcPrntIDs.size(); idCnt++) {
									if (!(isAvailableInList(lstMtchPrntIDs, lstSrcPrntIDs.get(idCnt)))) {
										logger.error("ERROR : " + srcEntityName + ">>" + srcAttrName + ">>" + (idCnt + 1) + " is not a valid value.");
										wEx.writeError(srcEntityName, idCnt + 1, srcColNum, "This is not a valid value, Please correct. This is foreign key of "	+ mtchEntityName + ">>" + mtchAttrName);
									}
								}
